...
为降低维护成本和用户学习成本，新增或调整功能页面时需要遵循以下约定：

- 页面采用统一的三栏布局：左侧放输入和输入素材预览，中间放生成/处理配置，右侧放输出、进度、下载和发布操作。
- 输入或上传本地素材后，应在对应输入控件下方直接校验并预览素材，不额外放到右侧输出区域，也不再单独做“素材预览”折叠区。
- 输入本地视频路径的功能应在输入框下方直接校验并预览原视频，例如「影视解说」和「录屏处理」都应把原视频预览放在左侧输入区域。
- “服务来源”用于区分能力由谁执行，统一命名为「LLM 视觉分析」「RunningHub 云端工作流」「本地 ComfyUI 工作流」「阿里云百炼/通义万相」。
- 「配音合成」保持同一套 UI：配音引擎统一为「Edge TTS 本地合成 / CosyVoice 本地模型 / ComfyUI 工作流合成」；Edge TTS 本地合成只展示可用中文音色；CosyVoice 本地模型安装到 `tools/cosyvoice/` 并通过独立 Python 环境直接推理，不依赖 ComfyUI；ComfyUI 工作流合成展示 `tts_*.json` 工作流音色，可接入 IndexTTS、SparkTTS 等。
- CosyVoice 的启用、检测和安装统一放在「系统配置 > 配音合成配置」中，业务页面只负责使用；如果未安装，业务页面提示回到系统配置处理。
- 背景音乐使用统一的 BGM 选择和音量控制，从 `bgm/` 与 `data/bgm/` 自动识别。
- 各功能可以保留业务特有开关，但不能改变共享模块的基本交互。比如「录屏处理」启用配音后应使用新配音替换原录屏音频，并可额外混入背景音乐；不提供“保留原声作为背景”的配音开关。
- 「录屏处理」必须以同一条时间轴驱动视频、字幕和配音：如果启用智能压缩，需要先生成压缩后的视频时间轴，再把字幕和配音一起映射到该时间轴，避免画面、字幕、声音不同步。
...
「录屏处理」用于处理已经录制好的本地视频，目标是识别原视频语音、生成/修正字幕、可选重新合成配音，并输出两个结果：

- 已处理的视频：字幕已烧录，音频、字幕、画面使用同一条时间轴。
- 剪映素材包：包含原视频、节奏处理后但未烧字幕/未混音的视频底板、SRT/ASS 字幕、ASR 结果、可选配音音频、背景音乐和术语修正规则。

#### 输入和字幕修正

- **本地录制视频地址**：填写运行 Pixelle-Video 的机器上可访问的视频路径。
- **业务专有名词 Markdown**：用于定义标准术语和常见误识别写法。
- **多音词/ASR 修正 Markdown**：用于把固定错误识别替换为正确文本。
- **行业/业务背景**：用于 AI 字幕润色，让字幕更符合所在行业表达。
- **识别模型**：模型越大识别效果通常越好，但处理更慢；默认 `small` 适合多数中文录屏。

#### 节奏处理

录屏处理的核心原则是：最终视频、字幕、配音必须共用同一条时间轴，不能让配音、字幕、画面各自独立变速。

**保持原时长**

- 输出视频时长与原视频基本一致。
- 字幕使用原 ASR 时间轴。
- 如果启用配音合成，配音会按原字幕时间轴插入；某句配音过长时会被压进该字幕段，避免挤占下一句。
- 配音不再按每条短字幕逐句合成，而是先把相邻字幕合成自然语音组，再把组内字幕按比例对齐到这段音频，减少机械断句和句间空白。
- 适合只做字幕、术语修正、轻量重配音的录屏。

**智能压缩**

- 系统根据 ASR 语音段生成新的压缩时间轴。
- 相邻语音段之间的空白超过「剪掉静音阈值」时，中间空白会被裁掉。
- 每个有效语音片段前后会按「片段前后保留」保留缓冲，避免剪辑过硬。
- 视频会先按压缩时间轴裁剪拼接，然后字幕和配音一起映射到压缩后时间轴。
- 启用配音时，智能压缩会按自然语音组生成配音，并用同一组时间轴裁剪画面和重排字幕，避免一句话读完后字幕仍长时间停留。
- 最终时长由实际保留的有效片段决定，不是简单按 TTS 倍速计算。

#### 配音语速和最终时长

- **配音语速**只影响 TTS 听感，不直接决定最终视频时长。
- 默认 `1.2x`，适合让录屏讲解更紧凑，但不会单独把 6 分钟视频变成 5 分钟或 3 分钟。
- 如果需要缩短最终视频，应使用「智能压缩」裁掉无效空白，而不是只提高配音语速。
- 如果未来需要固定输出时长（例如 6 分钟压到 3 分钟），应增加独立的「目标时长/整体变速」配置，由统一时间轴同时控制视频、字幕和配音。

#### 背景音乐和原声

- 启用配音合成后，原录屏音频不会作为背景保留。
- 背景音乐从 `bgm/` 与 `data/bgm/` 自动识别，可选择后混入新配音。
- 录屏处理的背景音乐默认音量为 `10%`，避免压过讲解声音。

#### 剪映素材包结构

- `01_original_video/`: 原始录屏视频。
- `02_timeline_video/`: 已完成节奏处理的视频底板，不烧录字幕，不混入配音和背景音乐。
- `03_subtitles/`: 与 `02_timeline_video/` 对齐的 SRT/ASS 字幕，可在剪映中单独修改文字。
- `04_transcript/`: ASR 原始结果与清洗后的分段。
- `05_dubbing/`: 可选配音合成音频，作为独立音轨导入剪映。
- `06_rules/`: 本次使用的术语/ASR 修正规则。
- `07_bgm/`: 本次选择的背景音乐，作为独立音轨导入剪映。
